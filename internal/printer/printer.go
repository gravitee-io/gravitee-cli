...
func New(format string, out io.Writer, quiet bool) *Printer {
	return &Printer{
		Format: format,
		Out:    out,
		Quiet:  quiet,
...
func (p *Printer) PrintList(items interface{}, columns []Column) error {
	if p.Quiet {
		return nil
	}
...
func (p *Printer) PrintDetail(item interface{}) error {
	if p.Quiet {
		return nil
	}
