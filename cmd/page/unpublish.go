...
func newUnpublishCmd(f *factory.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  gio page unpublish dddd1111-2222-3333-4444-555566667777 --api 8a7b3c4d-1234-5678-abcd-ef0123456789`,
		...
		RunE: func(_ *cobra.Command, args []string) error {
			...
			if err := cmdutil.CheckReadOnly(f, "page unpublish"); err != nil {
				return err
			}
...
func runUnpublish(f *factory.Factory, apiID, pageID string) error {
	path := cmdutil.V2EnvPath(f, fmt.Sprintf("apis/%s/pages/%s/_unpublish", apiID, pageID))

	data, err := f.Client.Post(path, nil)
	if err != nil {
		return fmt.Errorf("page unpublish failed: %w", err)
	}
