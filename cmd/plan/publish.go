...
func newPublishCmd(f *factory.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  gio plan publish dddd1111-2222-3333-4444-555566667777 --api 8a7b3c4d-1234-5678-abcd-ef0123456789`,
		...
		RunE: func(_ *cobra.Command, args []string) error {
			...
			if err := cmdutil.CheckReadOnly(f, "plan publish"); err != nil {
				return err
			}
...
func runPublish(f *factory.Factory, apiID, planID string) error {
	path := cmdutil.V2EnvPath(f, fmt.Sprintf("apis/%s/plans/%s/_publish", apiID, planID))

	data, err := f.Client.Post(path, nil)
	if err != nil {
		return fmt.Errorf("plan publish failed: %w", err)
	}
