...
func newResumeCmd(f *factory.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  gio subscription resume 34f8c9e7 --api 8a7b3c4d`,
		...
		RunE: func(_ *cobra.Command, args []string) error {
			...
			if err := cmdutil.CheckReadOnly(f, "subscription resume"); err != nil {
				return err
			}
...
func runResume(f *factory.Factory, apiID, subID string) error {
	path := cmdutil.V2EnvPath(f, fmt.Sprintf("apis/%s/subscriptions/%s/_resume", apiID, subID))

	data, err := f.Client.Post(path, nil)
	if err != nil {
		return fmt.Errorf("subscription resume failed: %w", err)
	}
