...
func newCreateCmd(f *factory.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  gio metadata create --api 8a7b3c4d-1234-5678-abcd-ef0123456789 -f metadata.json`,
		Args:    cobra.NoArgs,
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			if err := cmdutil.CheckReadOnly(f, "metadata create"); err != nil {
				return err
			}
...
func runCreate(f *factory.Factory, apiID, file string) error {
	...
	path := cmdutil.V2EnvPath(f, fmt.Sprintf("apis/%s/metadata", apiID))
