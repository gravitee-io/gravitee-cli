...
func newCreateCmd(f *factory.Factory, domainID *string) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "create [-f <file>]",
		Short: "Create an auth device notifier from a JSON file or stdin",
		Example: `  gctl am auth-device-notifier create --domain my-domain --file adn.json
  gctl am auth-device-notifier create --domain my-domain -f adn.json
  envsubst < adn.json | gctl am auth-device-notifier create --domain my-domain`,
		Args: cobra.NoArgs,
...
func runCreate(f *factory.Factory, domainID, file string) error {
	...
	data, err := f.AM().CreateAuthDeviceNotifier(domainID, body)
	if err != nil {
		return err
	}
