...
func newCreateCmd(f *factory.Factory, domainID *string) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  gio am auth-device-notifier create --domain my-domain --file adn.json
  gio am auth-device-notifier create --domain my-domain -f adn.json
  envsubst < adn.json | gio am auth-device-notifier create --domain my-domain`,
		Args: cobra.NoArgs,
