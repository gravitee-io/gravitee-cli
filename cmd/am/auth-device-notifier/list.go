...
func newListCmd(f *factory.Factory, domainID *string) *cobra.Command {
	return &cobra.Command{
		Use:     "list",
		Short:   "List auth device notifiers",
		Example: `  gctl am auth-device-notifier list --domain my-domain`,
		Args:    cobra.NoArgs,
...
func runList(f *factory.Factory, domainID string) error {
	items, err := f.AM().ListAuthDeviceNotifiers(domainID)
	if err != nil {
		return err
	}
	...
	return p.PrintList(items, authDeviceNotifierColumns())
}

func authDeviceNotifierColumns() []printer.Column {
