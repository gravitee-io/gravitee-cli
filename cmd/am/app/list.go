...
func newListCmd(f *factory.Factory, domainID *string) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  gctl am app list --domain my-domain
  gctl am app list --domain my-domain --query myapp --per-page 20`,
		Args: cobra.NoArgs,
...
func (o *listOptions) params(page int) am.ListApplicationsParams {
	return am.ListApplicationsParams{
		Query:   o.query,
		Page:    page,
		PerPage: o.perPage,
	}
...
func (o *listOptions) fetchPage(f *factory.Factory, p *printer.Printer, page int) error {
	resp, err := f.AM().ListApplications(*o.domainID, o.params(page-1)) // Convert 1-based CLI page to 0-based API page
	if err != nil {
		return err
	}
...
func (o *listOptions) fetchAll(f *factory.Factory, p *printer.Printer) error {
	allData, err := am.FetchAllPages(func(page int) (*am.PaginatedResponse, error) {
		return f.AM().ListApplications(*o.domainID, o.params(page))
	}, o.perPage)
	if err != nil {
		return err
	}
