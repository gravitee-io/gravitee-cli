...
func newCreateCmd(f *factory.Factory, domainID *string) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  gio am factor create --domain my-domain --file factor.json
  gio am factor create --domain my-domain -f factor.json
  envsubst < factor.json | gio am factor create --domain my-domain`,
		Args: cobra.NoArgs,
