...
func newCreateCmd(f *factory.Factory, domainID *string) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "create [-f <file>]",
		Short: "Create a factor from a JSON file or stdin",
		Example: `  gctl am factor create --domain my-domain --file factor.json
  gctl am factor create --domain my-domain -f factor.json
  envsubst < factor.json | gctl am factor create --domain my-domain`,
		Args: cobra.NoArgs,
...
func runCreate(f *factory.Factory, domainID, file string) error {
	...
	data, err := f.AM().CreateFactor(domainID, body)
	if err != nil {
		return err
	}
