...
func newGetCmd(f *factory.Factory, domainID *string) *cobra.Command {
	return &cobra.Command{
		Use:     "get <factorID>",
		Short:   "Get factor details",
		Example: `  gctl am factor get my-factor-id --domain my-domain`,
...
func runGet(f *factory.Factory, domainID, factorID string) error {
	data, err := f.AM().GetFactor(domainID, factorID)
	if err != nil {
		return err
	}
