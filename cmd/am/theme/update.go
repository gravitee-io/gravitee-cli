...
func newUpdateCmd(f *factory.Factory, domainID *string) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "update <themeID> --file <config.json>",
		Short: "Update a theme from a JSON file",
...
func runUpdate(f *factory.Factory, domainID, themeID, file string) error {
	body, err := cmdutil.ReadJSONFile(file)
	if err != nil {
		return err
	}

	data, err := f.AM().UpdateTheme(domainID, themeID, body)
	if err != nil {
		return err
	}
