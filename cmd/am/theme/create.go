...
func newCreateCmd(f *factory.Factory, domainID *string) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  gio am theme create --domain my-domain --file theme.json
  gio am theme create --domain my-domain -f theme.json
  envsubst < theme.json | gio am theme create --domain my-domain`,
		Args: cobra.NoArgs,
