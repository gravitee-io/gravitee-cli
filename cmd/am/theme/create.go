...
func newCreateCmd(f *factory.Factory, domainID *string) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "create [-f <file>]",
		Short: "Create a theme from a JSON file or stdin",
		Example: `  gctl am theme create --domain my-domain --file theme.json
  gctl am theme create --domain my-domain -f theme.json
  envsubst < theme.json | gctl am theme create --domain my-domain`,
		Args: cobra.NoArgs,
...
func runCreate(f *factory.Factory, domainID, file string) error {
	...
	data, err := f.AM().CreateTheme(domainID, body)
	if err != nil {
		return err
	}
