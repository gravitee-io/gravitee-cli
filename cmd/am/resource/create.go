...
func newCreateCmd(f *factory.Factory, domainID *string) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  gio am resource create --domain my-domain --file resource.json
  gio am resource create --domain my-domain -f resource.json
  envsubst < resource.json | gio am resource create --domain my-domain`,
		Args: cobra.NoArgs,
