...
func newCreateCmd(f *factory.Factory, domainID *string) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "create [-f <file>]",
		Short: "Create a resource from a JSON file or stdin",
		Example: `  gctl am resource create --domain my-domain --file resource.json
  gctl am resource create --domain my-domain -f resource.json
  envsubst < resource.json | gctl am resource create --domain my-domain`,
		Args: cobra.NoArgs,
...
func runCreate(f *factory.Factory, domainID, file string) error {
	...
	data, err := f.AM().CreateResource(domainID, body)
	if err != nil {
		return err
	}
