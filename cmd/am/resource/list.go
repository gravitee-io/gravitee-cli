...
func newListCmd(f *factory.Factory, domainID *string) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  gctl am resource list --domain my-domain
  gctl am res list --domain my-domain`,
		Args: cobra.NoArgs,
...
func runList(f *factory.Factory, domainID string) error {
	items, err := f.AM().ListResources(domainID)
	if err != nil {
		return err
	}
	...
	return p.PrintList(items, resourceColumns())
}

func resourceColumns() []printer.Column {
