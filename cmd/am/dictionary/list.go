...
func newListCmd(f *factory.Factory, domainID *string) *cobra.Command {
	return &cobra.Command{
		Use:     "list",
		Short:   "List i18n dictionaries",
		Example: `  gctl am dictionary list --domain my-domain`,
		Args:    cobra.NoArgs,
...
func runList(f *factory.Factory, domainID string) error {
	items, err := f.AM().ListDictionaries(domainID)
	if err != nil {
		return err
	}
	...
	return p.PrintList(items, dictionaryColumns())
}

func dictionaryColumns() []printer.Column {
	return []printer.Column{
		...
		{Name: "Locale", Value: func(i any) string { return cmdutil.StringField(i, "locale") }},
	}
}
