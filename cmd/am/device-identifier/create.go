...
func newCreateCmd(f *factory.Factory, domainID *string) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "create [-f <file>]",
		Short: "Create a device identifier from a JSON file or stdin",
		Example: `  gctl am device-identifier create --domain my-domain --file device-identifier.json
  gctl am device-identifier create --domain my-domain -f device-identifier.json
  envsubst < device-identifier.json | gctl am device-identifier create --domain my-domain`,
		Args: cobra.NoArgs,
