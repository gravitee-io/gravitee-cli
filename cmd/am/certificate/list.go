...
func newListCmd(f *factory.Factory, domainID *string) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  gctl am certificate list --domain my-domain
  gctl am cert list --domain my-domain`,
		Args: cobra.NoArgs,
...
func runList(f *factory.Factory, domainID string) error {
	items, err := f.AM().ListCertificates(domainID)
	if err != nil {
		return err
	}
	...
	return p.PrintList(items, certColumns())
}

func certColumns() []printer.Column {
	return []printer.Column{
		...
		{Name: "ExpiresAt", Value: func(i any) string { return cmdutil.StringField(i, "expiresAt") }},
	}
}
