...
func newCreateCmd(f *factory.Factory, domainID *string) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "create [-f <file>]",
		Short: "Create a form from a JSON file or stdin",
		Example: `  gctl am form create --domain my-domain --file form.json
  gctl am form create --domain my-domain -f form.json
  envsubst < form.json | gctl am form create --domain my-domain`,
		Args: cobra.NoArgs,
...
func runCreate(f *factory.Factory, domainID, file string) error {
	...
	data, err := f.AM().CreateForm(domainID, body)
	if err != nil {
		return err
	}
