...
func newListCmd(f *factory.Factory, domainID *string) *cobra.Command {
	return &cobra.Command{
		Use:     "list",
		Short:   "List authorization engines",
		Example: `  gctl am authorization-engine list --domain my-domain`,
		Args:    cobra.NoArgs,
...
func runList(f *factory.Factory, domainID string) error {
	items, err := f.AM().ListAuthorizationEngines(domainID)
	if err != nil {
		return err
	}
	...
	return p.PrintList(items, authorizationEngineColumns())
}

func authorizationEngineColumns() []printer.Column {
