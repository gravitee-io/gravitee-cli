...
func newUpdateCmd(f *factory.Factory, domainID *string) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "update <engineID> [-f <file>]",
		Short: "Update an authorization engine from a JSON file or stdin",
		Example: `  gctl am authorization-engine update my-engine-id --domain my-domain --file engine.json
  gctl am authorization-engine update my-engine-id --domain my-domain -f engine.json
  envsubst < engine.json | gctl am authorization-engine update my-engine-id --domain my-domain`,
