...
func newCreateCmd(f *factory.Factory, domainID *string) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  gio am bot-detection create --domain my-domain --file bot-detection.json
  gio am bot-detection create --domain my-domain -f bot-detection.json
  envsubst < bot-detection.json | gio am bot-detection create --domain my-domain`,
		Args: cobra.NoArgs,
