...
func newCreateCmd(f *factory.Factory, domainID *string) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "create [-f <file>]",
		Short: "Create a bot detection from a JSON file or stdin",
		Example: `  gctl am bot-detection create --domain my-domain --file bot-detection.json
  gctl am bot-detection create --domain my-domain -f bot-detection.json
  envsubst < bot-detection.json | gctl am bot-detection create --domain my-domain`,
		Args: cobra.NoArgs,
...
func runCreate(f *factory.Factory, domainID, file string) error {
	...
	data, err := f.AM().CreateBotDetection(domainID, body)
	if err != nil {
		return err
	}
