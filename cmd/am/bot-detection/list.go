...
func newListCmd(f *factory.Factory, domainID *string) *cobra.Command {
	return &cobra.Command{
		Use:     "list",
		Short:   "List bot detections",
		Example: `  gctl am bot-detection list --domain my-domain`,
		Args:    cobra.NoArgs,
...
func runList(f *factory.Factory, domainID string) error {
	items, err := f.AM().ListBotDetections(domainID)
	if err != nil {
		return err
	}
	...
	return p.PrintList(items, botDetectionColumns())
}

func botDetectionColumns() []printer.Column {
