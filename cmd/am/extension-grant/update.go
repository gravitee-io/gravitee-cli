...
func newUpdateCmd(f *factory.Factory, domainID *string) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "update <grantID> [-f <file>]",
		Short: "Update an extension grant from a JSON file or stdin",
		Example: `  gctl am extension-grant update my-grant-id --domain my-domain --file grant.json
  gctl am extension-grant update my-grant-id --domain my-domain -f grant.json
  envsubst < grant.json | gctl am extension-grant update my-grant-id --domain my-domain`,
