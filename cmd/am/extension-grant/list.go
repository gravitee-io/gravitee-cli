...
func newListCmd(f *factory.Factory, domainID *string) *cobra.Command {
	return &cobra.Command{
		Use:   "list",
		Short: "List extension grants",
		Example: `  gctl am extension-grant list --domain my-domain
  gctl am eg list --domain my-domain`,
		Args: cobra.NoArgs,
...
func runList(f *factory.Factory, domainID string) error {
	items, err := f.AM().ListExtensionGrants(domainID)
	if err != nil {
		return err
	}
	...
	return p.PrintList(items, extensionGrantColumns())
}

func extensionGrantColumns() []printer.Column {
	return []printer.Column{
		...
		{Name: "GrantType", Value: func(i any) string { return cmdutil.StringField(i, "grantType") }},
	}
}
