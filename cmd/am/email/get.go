...
func newGetCmd(f *factory.Factory, domainID *string) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  gctl am email get my-email-id --domain my-domain`,
...
func runGet(f *factory.Factory, domainID, emailID string) error {
	data, err := f.AM().GetEmail(domainID, emailID)
	if err != nil {
		return err
	}
...
func printEmailDetail(p *printer.Printer, data []byte) error {
