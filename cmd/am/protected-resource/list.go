...
func newListCmd(f *factory.Factory, domainID *string) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  gctl am protected-resource list --domain my-domain`,
		Args:    cobra.NoArgs,
...
func runList(f *factory.Factory, domainID string) error {
	items, err := f.AM().ListProtectedResources(domainID)
	if err != nil {
		return err
	}
	...
	return p.PrintList(items, protectedResourceColumns())
}

func protectedResourceColumns() []printer.Column {
