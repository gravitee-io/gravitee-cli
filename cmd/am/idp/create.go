...
func newCreateCmd(f *factory.Factory, domainID *string) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  gio am idp create --domain my-domain --file idp.json
  gio am idp create --domain my-domain -f idp.json
  envsubst < idp.json | gio am idp create --domain my-domain`,
		Args: cobra.NoArgs,
