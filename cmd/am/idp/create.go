...
func newCreateCmd(f *factory.Factory, domainID *string) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "create [-f <file>]",
		Short: "Create an identity provider from a JSON file or stdin",
		Example: `  gctl am idp create --domain my-domain --file idp.json
  gctl am idp create --domain my-domain -f idp.json
  envsubst < idp.json | gctl am idp create --domain my-domain`,
		Args: cobra.NoArgs,
...
func runCreate(f *factory.Factory, domainID, file string) error {
	...
	data, err := f.AM().CreateIdentityProvider(domainID, body)
	if err != nil {
		return err
	}
